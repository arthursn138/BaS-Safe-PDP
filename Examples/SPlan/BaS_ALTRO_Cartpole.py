...
horizon = 150
init_state = [0, 0, 0, 0]
init_state += [1/(max_x ** 2 - init_state[0] ** 2)]
dyn = env.X + dt * env.f
...
if True:
    save_data = {'control_traj_trace': control_traj_trace,
                 ...
                 'base_mu': base_mu,
                 'lr': lr}
    np.save('./Results/ALTRO_BaS_Cartpole_trial_1.npy', save_data)
    sio.savemat('./Results/ALTRO_BaS_Cartpole_Testing_lim_' + str(max_x) + '.mat', {'results': save_data})


# plot and animation
sol = altro.integrateSys(init_state, control_traj)
print(sol['cost'])
# env.play_animation(pole_len=2, dt=dt, state_traj=sol['state_traj'], save_option=1, title='BaS-ALTRO')
